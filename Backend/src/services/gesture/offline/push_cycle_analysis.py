...
def _percentile(values: list[float], probability: float) -> float | None:
    ...
    position = max(0.0, min(float(len(ordered) - 1), probability * float(len(ordered) - 1)))
    ...
    *,
    ...
    def is_active(sample: PushFrameSample) -> bool:
        ...
        if sample.push_depth < activation_depth_threshold:
            return False
        if sample.index_extension_ratio is not None and sample.index_extension_ratio < min_index_extension_ratio:
            return False
        if sample.center_distance is not None and sample.center_distance > center_distance_max:
            return False
    ...
    def finalize(end_index: int) -> None:
        ...
        pose_valid_ratio = mean(1.0 if sample.pose_valid else 0.0 for sample in active_segment)
        ...
        active_times = [sample.timestamp for sample in samples[active_start : active_last_index + 1] if is_active(sample)]
        hold_duration_seconds = max(0.0, active_times[-1] - active_times[0]) if len(active_times) >= 2 else 0.0
        observed_gesture: PushGestureName = "push_click_long" if hold_duration_seconds >= long_click_seconds else "push_click_short"
        average_hand_sizes = [sample.hand_size for sample in segment if sample.hand_size is not None]
        cycles.append(
            PushCycle(
                ...
                average_hand_size=mean(average_hand_sizes) if average_hand_sizes else None,
    ...
    for index, sample in enumerate(samples):
        ...
        if sample.push_depth > release_depth_threshold and gap_seconds <= inactive_grace_seconds:
            continue

        if sample.push_depth <= release_depth_threshold or gap_seconds > active_gap_seconds:
            finalize(index)
...
def profile_push_cycles(
    ...
    for cycle_index, cycle in enumerate(cycles):
        ...
        depth_stability_values = [depth for depth in cycle.push_depths if depth >= forward_depth * 0.7]
        depth_stability = max(depth_stability_values, default=0.0) - min(depth_stability_values, default=0.0)
        pose_valid_ratio = mean(1.0 if pose_valid else 0.0 for pose_valid in cycle.pose_valid_samples)
        extension_values = [sample.index_extension_ratio for sample in segment if sample.index_extension_ratio is not None]
        center_values = [sample.center_distance for sample in segment if sample.center_distance is not None]
        folded_values = [float(sample.folded_fingers_count) for sample in segment]
        active_times = [sample.timestamp for sample in segment if sample.pose_valid and sample.push_depth >= forward_depth * 0.7]
        hold_duration_seconds = max(0.0, active_times[-1] - active_times[0]) if len(active_times) >= 2 else 0.0
        lead_in_idle_seconds = max(0.0, cycle.start_time - samples[cycle.start_index - 1].timestamp) if cycle.start_index > 0 else 0.0
        settle_idle_seconds = max(0.0, samples[cycle.end_index + 1].timestamp - cycle.end_time) if cycle.end_index < len(samples) - 1 else 0.0
        profiles.append(
            PushCycleProfile(
                ...
                index_extension_mean=mean(extension_values) if extension_values else None,
