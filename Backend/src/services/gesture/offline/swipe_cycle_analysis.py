...
def segment_swipe_cycles(
    ...
    *,
    ...
    for index in range(len(samples) - 1):
        ...
        if abs(axis_delta) >= motion_step_threshold and abs(axis_delta) >= abs(off_axis_delta) * axis_ratio_threshold:
    ...
    for point_index, step_sign in step_end_indices[1:]:
        ...
        if step_sign == grouped_sign and point_index - previous_index <= 2 and time_gap <= active_gap_seconds:
    ...
    for grouped_sign, grouped_indices in grouped_end_indices:
        ...
        while start_index > last_end_index + 1:
            ...
            if previous_speed < edge_speed_threshold:
                break
            if samples[start_index].timestamp - samples[previous_index].timestamp > edge_gap_seconds:
                break
        ...
        while end_index < len(samples) - 1:
            ...
            if current_speed < edge_speed_threshold:
                break
            if samples[end_index + 1].timestamp - samples[end_index].timestamp > edge_gap_seconds:
                break
        ...
        timestamps = [sample.timestamp for sample in samples[start_index : end_index + 1]]
        average_hand_size_values = [sample.hand_size for sample in samples[start_index : end_index + 1] if sample.hand_size is not None]
        average_hand_size = mean(average_hand_size_values) if average_hand_size_values else None
        ...
        off_axis_displacement = abs(features.dy_total) if axis == "x" else abs(features.dx_total)
...
def profile_swipe_cycles(
    ...
    for cycle_index, cycle in enumerate(cycles):
        ...
        signed_displacement = features.dx_total * cycle.sign if cycle.axis == "x" else features.dy_total * cycle.sign
        off_axis_displacement = abs(features.dy_total) if cycle.axis == "x" else abs(features.dx_total)
        ...
        normalized_signed_displacement = signed_displacement / max(features.hand_size_scale, 1e-6)
...
def summarize_swipe_profiles(
    ...
    normalized_displacements = [profile.normalized_signed_displacement for profile in profiles]
    ...
    return {
        ...
        "normalized_signed_displacement_p10": _percentile(normalized_displacements, 0.10),
