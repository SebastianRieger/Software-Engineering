...
class MediaPipeHandsAdapter:
	...
	def list_available_cameras(max_devices: int = 8) -> list[dict[str, str | int | bool | None]]:
		...
		devices_by_key: dict[str, dict[str, str | int | bool | None]] = {}
		...
		return sorted(
			...
			key=lambda device: int(device.get("index", 0)) if isinstance(device.get("index"), int) else 0,
		)

	@staticmethod
	def _list_linux_cameras(indices: list[int]) -> list[dict[str, str | int | bool | None]]:
		devices_by_key: dict[str, dict[str, str | int | bool | None]] = {}
