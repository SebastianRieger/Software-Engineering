...
FingerName = str
CameraDeviceInfo = dict[str, object]
...
class MediaPipeHandsAdapter:
    ...
    @staticmethod
    def list_available_cameras(max_devices: int = 8) -> list[CameraDeviceInfo]:
        if cv2 is None:
            return []

        linux_video_indices = MediaPipeHandsAdapter._linux_video_indices()
        if linux_video_indices:
            return MediaPipeHandsAdapter._list_linux_cameras(linux_video_indices)

        candidate_indices = list(range(max_devices))
        ...
        for index in candidate_indices:
            ...
            device_key = MediaPipeHandsAdapter._camera_device_key(index)
            existing = devices_by_key.get(device_key)
            if existing is None:
            ...
            existing_available = bool(existing.get("available"))
            existing_index = existing.get("index")
            should_replace_index = False
            if not existing_available and available:
                should_replace_index = True
            elif isinstance(existing_index, int) and index < existing_index:
                should_replace_index = True

            if should_replace_index:
                existing["index"] = index
                existing["name"] = MediaPipeHandsAdapter._resolve_camera_name(index)
                if backend_name is not None:
                    existing["backend"] = backend_name

            existing["available"] = existing_available or available
            if existing.get("backend") is None and backend_name is not None:
                existing["backend"] = backend_name

        return sorted(
            devices_by_key.values(),
            key=MediaPipeHandsAdapter._camera_sort_index,
        )

    @staticmethod
    def _list_linux_cameras(indices: list[int]) -> list[CameraDeviceInfo]:
        devices_by_key: dict[str, CameraDeviceInfo] = {}
        for index in indices:
            device_key = MediaPipeHandsAdapter._camera_device_key(index)
            device_payload: CameraDeviceInfo = {
                "index": index,
                "name": MediaPipeHandsAdapter._resolve_camera_name(index),
                "available": MediaPipeHandsAdapter._linux_video_device_exists(index),
                "backend": None,
            }
            existing = devices_by_key.get(device_key)
            if existing is None:
                devices_by_key[device_key] = device_payload
                continue
    ...
    @staticmethod
    def _camera_sort_index(device: CameraDeviceInfo) -> int:
        index = device.get("index", 0)
        return index if isinstance(index, int) else 0

    @staticmethod
    def _camera_device_key(index: int) -> str:
        sysfs_device_path = Path(f"/sys/class/video4linux/video{index}/device")
        if sysfs_device_path.exists():
            return os.path.realpath(sysfs_device_path)
        return f"video-node:{index}"

    @staticmethod
    def _linux_video_device_exists(index: int) -> bool:
        device_path = Path(f"/dev/video{index}")
        return device_path.exists() and os.access(device_path, os.R_OK)
...
__all__ = [
    ...
    "normalize_tracked_hand_observation",
    "smooth_point",
]
