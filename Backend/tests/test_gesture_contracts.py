...
def test_gesture_contracts_define_recording_profiles_for_core_gestures():
    ...
    assert contracts["push_click_long"].tempo_hint == "langsamer commit plus sichtbare haltephase"
    assert contracts["swipe_up"].start_pose == "offene handflaeche in der mittleren kamerazone"
...
def test_default_gesture_specs_are_derived_from_contracts():
    ...
    assert specs["push_click_long"].required_primitives == contracts["push_click_long"].required_primitives
