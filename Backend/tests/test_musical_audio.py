...
class StaticMusicalAudioRepository:
    def __init__(self):
        self.musical_audio_config = MusicalAudioConfig(
            enabled=True, active_artifact_id="whistle-main"
        )
        self.artifacts = [
            MusicalAudioTrainingArtifact(
                ...
                notes=[
                    MusicalAudioNoteEvent(
                        relative_pitch_semitones=0.0, relative_time_seconds=0.0
                    ),
                    MusicalAudioNoteEvent(
                        relative_pitch_semitones=2.0, relative_time_seconds=0.3
                    ),
                    MusicalAudioNoteEvent(
                        relative_pitch_semitones=4.0, relative_time_seconds=0.6
                    ),
                ],
                match_threshold=0.6,
                sample_count=14,
            ...
            update={
                "modality_settings": {
                    **self.command_profile.modality_settings,
                    "musical_audio": self.command_profile.modality_settings[
                        "musical_audio"
                    ].model_copy(
                        update={
                            "enabled": enabled,
...
@pytest.mark.asyncio
async def test_start_musical_audio_returns_clear_status_error(
    client, override_musical_audio_dependency
):
    _ = override_musical_audio_dependency
    response = await client.post(
        "/api/v1/musical-audio/start", json={"device_index": 1}
    )
...
def test_musical_audio_service_matches_note_events_and_publishes_semantic_action():
    ...
    accepted = service.process_note_events(
        [
            MusicalAudioNoteEvent(
                relative_pitch_semitones=0.0, relative_time_seconds=0.0, confidence=0.95
            ),
            MusicalAudioNoteEvent(
                relative_pitch_semitones=2.0,
                relative_time_seconds=0.31,
                confidence=0.93,
            ),
            MusicalAudioNoteEvent(
                relative_pitch_semitones=4.0,
                relative_time_seconds=0.62,
                confidence=0.91,
            ),
        ]
    )
...
def test_musical_audio_service_start_reports_invalid_sample_rate_without_fallback(
    monkeypatch,
):
