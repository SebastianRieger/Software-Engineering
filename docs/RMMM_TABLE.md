...
| R-02 | Technical | Frontend und Backend sind nur teilweise integriert. Der aktuelle produktive Schnitt deckt Wetter und Konfiguration ab, während gemeinsamer WebSocket, Kalender, Smart Home und Gesten-Consumer im Frontend noch fehlen. | 4 | 4 | 16 | Weiter in kleinen vertikalen Slices arbeiten, API-Verträge stabil halten, erst bestehende Domains ganz integrieren bevor neue Frontend-Features entstehen. | Frontend nutzt nur Wetter-/Config-Client; kein produktiver WebSocket-Consumer; weitere Backend-Domänen bleiben im UI unsichtbar. | Für Demo und Vorlesung nur den vorhandenen Verticalschnitt zeigen und fehlende Domänen explizit als Backend-Vorbereitung kennzeichnen. | Frontend + Backend | Mitigating | 2026-04-13 |
...
| R-07 | Quality | Im Frontend fehlen automatisierte Tests und eine konsolidierte State-Strategie für größere Erweiterungen. Das erhöht Regressionsrisiko bei Grid-, Widget- und späteren Realtime-Integrationen. | 4 | 4 | 16 | Vor nächsten größeren Frontend-Slices Testbasis und klaren State-Ansatz definieren; vorhandenen datengetriebenen Vue-Ansatz nicht wieder mit DOM-Manipulation unterlaufen. | Keine Test-Skripte in [../Frontend/nimrag-frontend/package.json](../Frontend/nimrag-frontend/package.json); Fehler werden erst manuell sichtbar. | Frontend-Umfang vorübergehend klein halten und Änderungen nur in schmalen, manuell verifizierten Slices ausliefern. | Frontend | Open | 2026-04-13 |
